...
class AscendBinary(AbstractLeaf):
    ...
    def encode(self, attribute, decoded, *args, **kwargs):
        #  unless specified throw kwargs, use the pyrad keystore to avoid
        #  causing breakages
        keystore = AbinaryKeystores.PYRAD
        if 'keystore_abinary' in kwargs:
            keystore = kwargs['keystore_abinary']

        match keystore:
            case AbinaryKeystores.PYRAD:
                terms = {
                    'family': b'\x01',
                    'action': b'\x00',
                    'direction': b'\x01',
            ...
            case AbinaryKeystores.FREERADIUS:
                terms = {
                    'srcip': b'=x00',
                    'dstip': None,
                    'srcmask': '',
            ...
            case _:
                raise ValueError('Unexpected abinary keystore name')
